      ...
      <el-form-item>
        <el-select v-model="loginForm.entryType" style="width:100%" @change="entryTypeChange">
          <el-option
            ...
            :key="item.id"
            :label="$dict.getNameById($dict.EntryType, item.id)"
            :value="item.id"
          />
        </el-select>
      </el-form-item>
      <div class="input-style">
        <el-form-item v-if="loginForm.entryType==='Tenant'" prop="tenantCode">
          <el-input
            ref="tenantCode"
...
export default {
  ...
  data() {
    const validateUsername = (rule, value, callback) => {
      if (value.length < 1) {
        callback(new Error(this.$t('login.请输入用户名')))
      } else {
        callback()
      }
    }
    const validatePassword = (rule, value, callback) => {
      if (value.length < 6) {
        callback(new Error(this.$t('login.密码最小长度为6')))
      } else {
        callback()
      }
    }
    ...
    return {
      ...
      loginRules: {
        ...
        tenantCode: [
          { required: true, trigger: 'blur', validator: validateTenantCode }
        ]
      },
